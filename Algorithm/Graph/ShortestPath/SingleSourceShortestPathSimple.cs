...
{
    public class SingleSourceShortestPathSimple<V> : SingleSourceShortestPath<V>
    {
        /*
         ...
         * tree) with given source as root. We maintain two sets, one set 
         * contains vertices included in shortest path tree, other set includes
         ...
         *    source) and weight of edge u-v, is less than the distance value 
         ...
         * infinite. Now pick the vertex with minimum distance value. The vertex
         * 0 is picked, include it in sptSet. So sptSet becomes {0}. After 
         ...
         * 1 and 7 are updated as 4 and 8. Following subgraph shows vertices and
         * their distance values, only the vertices with finite distance values
         * are shown. The vertices included in SPT are shown in green color.
         * 
         ...
         * distance value of vertex 6 and 8 becomes finite (15 and 9 
         * respectively).
         * 
         * Pick the vertex with minimum distance value and not already included 
         ...
         * 
         * We repeat the above steps until sptSet doesn’t include all vertices 
        ...
        */
        ...
        // shortestPath[i] will be true if vertex i is included in shortest
        ...
        private int FindMinimumDistance()
        {
            ...
            {
                if (!componentOfPath[v] && Distance[v] <= min)
                {
                    min = Distance[v];
                    min_index = v;
                }
        ...
        public override void ShortestPath()
        {
            ...
            {
                int up = FindMinimumDistance();
                componentOfPath[up] = true;
                var u = graph.GetVertexByID((uint)up); 

                foreach(var w in graph.GetNeighbours(v))
                {
                    // Already included as part of the path
                    if (componentOfPath[w.ID])
                        continue;
                    Relax(v, w);
                }
            }
