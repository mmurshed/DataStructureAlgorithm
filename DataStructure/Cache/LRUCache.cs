...
namespace DataStructure.Cache
{
     ...
     * 
     * We are given total possible page numbers that can be referred. We 
     * are also given cache (or memory) size (Number of page frames that 
     ...
     * referenced which is not there in cache. Please see the Galvin book 
     * for more details (see the LRU page replacement slide here).
     * 
     ...
     * update the corresponding node address in the hash. If the queue is 
     * full, i.e. all the frames are full, we remove a node from the rear 
     * of queue, and add the new node to the front of queue.
     * 
     ...
     * Explanation –
     * 
     * Given 3 page frames, so we take size of Queue is 3 initially.
     * Queue is empty.
     * ________________
     * |    |    |    |
     * ----------------
     ...
     * 
     * Input: 2
     * Every new input will be front as defined by LRU
     *   ________________
     *   | 2  | 1  |    |
     *  /--------\-------
     * /          \
     ...
     * 
     * Input 4:
     * Since all frames are full, we remove a node from the rear of 
     * the queue and add the new node to the front of the queue.
     *    ________________
     *    | 4  | 3  | 2  |
     *   /-------------\--
     *  /               \
     ...
     * 
     * Input 1:
     * Since present in memory, so bring it to the front of the 
     * queue. This is called hit.
     *     ________________
     *     | 1  | 5  | 2  |
     *    /-------------\--
     *   /               \
     ...
     * pointer           pointer
     * 
     * So, we have only 2 hits and 10 page faults using LRU page
    ...
    public class LRUCache
    {
        ...
        public void Refer(int x)
        {
            ...
            {
                node = new LinkedListNode<Node>(new Node(x, 0));
                dictionary[x] = node;
            }
        ...
        {
            int[] a = new int[4];
            Array.Copy(a, 0, base, 0, n)
            if (!dictionary.ContainsKey(key))
            {
                var node = new LinkedListNode<Node>(new Node(key, value));
                dictionary[key] = node;
