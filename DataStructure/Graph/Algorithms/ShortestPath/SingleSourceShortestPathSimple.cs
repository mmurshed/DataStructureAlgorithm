...
We use a boolean array sptSet[] to represent the set of vertices included in SPT. If a value sptSet[v] is true, then vertex v is included in SPT, otherwise not. Array dist[] is used to store shortest distance values of all vertices.
        */
        ...
        private int FindMinimumDistance()
        {
            ...
            {
                if (componentOfPath[v] == false && Distance[v] <= min)
                {
                    min = Distance[v];
                    min_index = v;
                }
        ...
        public override void ShortestPath()
        {
            ...
            foreach(var v in graph.Vertices)
            {
                ...
                foreach(var w in graph.GetNeighbours(v))
                {
                    ...
                    var edge = graph.GetEdge(u, w);
                    
                    var newDistance = Distance[up] + edge.Value;
                    // If the new distance is smaller than the previous distance calculated
                    if (newDistance < Distance[w.ID])
                    {
                        Distance[w.ID] = newDistance;
                        PreviousVertex[w.ID] = u;
                    }
                }
            }
        }
