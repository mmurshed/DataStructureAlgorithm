...
namespace Algorithm.DynamicProgramming
{
     ...
     * 
     * In share trading, a buyer buys shares and sells on future date. Given
     * stock price of n days, the trader is allowed to make at most k 
     * transactions, where new transaction can only start after previous 
     * transaction is complete, find out maximum profit that a share 
     ...
     * 
     * There are various versions of the problem. If we are allowed to buy 
     * and sell only once, then we can use Maximum difference between two 
     * elements algorithm. If we are allowed to make at most 2 transactions,
     * we can follow approach discussed here. If we are allowed to buy and 
     * sell any number of times, we can follow approach discussed here.
    */
    public class BuySellStocksKTimes
    {
        /*
         * In this post, we are only allowed to make at max k transactions. 
         * The problem can be solved by using dynamic programming.
         * 
         * Let profit[t, i] represent maximum profit using at most t 
         * transactions up to day i (including day i). Then the relation is:
         * 
         ...
         * 
         * 
         *                                               | Profit without transacting that day
         * Profit for transacation t and day i = Max of <|
         *                                               | Profit with transacting that day
         * 
         *                                               | Profit obtained previous day
         *                                     = Max of <|
         *                                               | Max profit for buying on each of previous day
         *                                               | and selling at day i
         *                                               | PLUS the profit of previous transaction at that day
         *
         *                                               | Profit obtained previous day
         *                                     = Max of <|
         *                                               | For buying on each of previous day j
         ...
         * on ith day, we need to purchase it on any one of [0, i – 1] days.
         * If we buy shares on jth day and sell it on ith day, max profit 
         * will be price[i] – price[j] + profit[t-1][j] where j varies from 0
         * to i-1. Here profit[t-1][j] is best we could have done with one 
         * less transaction till jth day.        
         */
        ...
        /*
         * Optimized Solution:
         * The above solution has time complexity of O(k.n2). It can be reduced
         * if we are able to calculate maximum profit gained by selling shares 
         * on ith day in constant time.
         * 
         ...
         * 
         * So, if we have already calculated max(profit[t-1, j] – price[j])
         * for all j in range [0, i-2], 
         * we can calculate it for j = i – 1 in constant time.
         * 
         * In other words, we don’t have to look back in range [0, i-1] anymore 
         * to find out best day to buy. We can determine that in constant time 
