...
Arithmetic Progression (AP). This process takes O(n^3) time.

We can solve this problem in O(n^2) time using Dynamic Programming. To get idea 
...
and fix every element as middle element. For an element set[j] to be middle of 
AP, there must exist elements ‘set[i]’ and ‘set[k]’ such that 
set[i] + set[k] = 2*set[j] where 0 <= i < j and j < k <=n-1. 
...
LLAP is at least 2, because 2 element will always create an
...
before as the loop traverses from right to left columns.
    */
    public class LongestArithmeticProgression
    {
        ...
        // which is SORTED
        public int LenghtOfLongestAP(int[] set)
        {
            int n = set.Length;
            if (n <= 2)
                return n;
