...
namespace Algorithm.DynamicProgramming
{
    ...
    /* 
     * Given a set of numbers, find the Length of the Longest Arithmetic 
     * Progression (LLAP) in it. A progression of numbers is arithmatic when
     * difference between the numbers are all equal.
     * 
     * Examples:
     * 
     * set[] = {1, 7, 10, 15, 27, 29}
     * output = 3
     * 
     * The longest arithmetic progression is {1, 15, 29}, difference 14.
     * i.e. {1, 1+14, 1+14+14}
     * 
     * set[] = {5, 10, 15, 20, 25, 30}
     * output = 6
     * 
     * The whole set is in AP, difference 5. 
     * 
     * For simplicity, we assumed that the given set is sorted. We can 
     ...
     * for the more elements in Arithmetic Progression (AP). This process takes
     * O(n^3) time. We can solve this problem in O(n^2) time using Dynamic 
     * Programming.
     * 
     * To get idea of the DP solution, let us first discuss solution of
     * following simpler problem.
     * 
     * Given a sorted set, find if there exist three elements in 
     ...
     * the second element and fix every element as middle element.
     * 
     * For an element set[j] to be middle of AP, there must exist elements 
     * set[i] and set[k] such that set[i] + set[k] = 2 * set[j] 
     * where 0 <= i < j and j < k <=n-1. 
     * 
     * How to efficiently find i and k for a given j?
     ...
     *    
     * Following is C# implementation of the above algorithm for the 
     * simpler problem.
     * 
     ...
     * original problem is Length of the Longest Arithmetic Progression (LLAP)
     * which is an integer value. If the given set has two or more elements, 
     * then the value of LLAP is at least 2, because 2 element will always 
     * create an arithmatic progression.
     * 
     * The idea is to create a 2D table L[n, n]. An entry L[i, j] in this table
     * stores LLAP with set[i] and set[j] as first two elements of AP and j > i.
     * The last column of the table is always 2 (Why – see the meaning of 
     * L[i, j]).
     * 
     * Rest of the table is filled from bottom right to top left. To fill rest
     * of the table, j (second element in AP) is first fixed. i and k are 
     * searched for a fixed j. 
     * 
     * If i and k are found such that i, j, k form an AP, then the value of 
     * L[i, j] is set as L[j, k] + 1. Note that the value of L[j, k] must 
     * have been filled before as the loop traverses from right to left columns.
     * 
    */
    public class LongestArithmeticProgression
    {
        // Assumption sorted
        // O(n^3)
        public int LengthOfLongestAPNaive(int[] set)
        {
            int n = set.Length;
            if (n <= 2)
                return 2;
        ...
        // which is SORTED
        public int LenghtOfLongestAP(int[] a)
        {
            int n = a.Length;
            if (n <= 2)
                return n;
